...
////////////////  Load dnde amplitude data
   ...
   TNtuple  dndef( "dndef","dNdE","row:amplitude:sigma");
    dndef.ReadFile( "../chains/dnde_mod1.dat"); 
...
for ( int j=i; j< sdim; j++)
    {
     spectrum2[i]=spectrum2[i]+spectrum0[j];
     }
 }
   ...
   ///////////////  best model
      ...
      float** bestdnde;
       float  spectrum[sdim];
 bestdnde= read_file("../chains/dnde_best.dat",sdimT,2);
        for ( int i =0; i< sdim; i++)
   {
     ii= sdim -i-1;
      spectrum[ii] = bestdnde[1][i];
      //cout<<"spectrum " <<i<<" "<<spectrum[i]<<endl;

      }
  shift=bestdnde[1][sdim];
   slp=bestdnde[1][sdim+1];
...
////////////////  Load step function flux data
    //cout<<"hahaha"<<endl;
	 ...
	 dmnum = 146;
         int  stepnum=12;
 stepffile= read_file("../chains/spectrum_stepf.dat",stepnum,dmnum);
     float DMflux[dmnum];
     float DMflux3[dmnum];
     float DMeng[dmnum];
      float epTotal[dmnum];
...
//     pad1->GetFrame()->SetFillColor(42);
  pad1-> SetLogx();
  pad1-> SetLogy();
   TGraphErrors *gr = new TGraphErrors(sdim,eng,spectrum0,NULL,sigma);
